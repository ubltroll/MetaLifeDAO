...
@use_redis(30, {'2':2592000, '3':2592000, '4':2592000, '7':2592000})
...
def get_address_dao(address):
    ...
    for dao_address in dao_addresses:
        try:
            ...
            continue
        res.append({'address': dao_address, 'name': dao.name, 'uri': dao.uri, 'version': dao.version})
    return res
...
def get_daos():
    ...
    for dao in DAOs.select().order_by(DAOs.createBlock.desc()):
        res.append({'address': dao.address, 'name': dao.name, 'uri': dao.uri, 'version': dao.version})
    return res
